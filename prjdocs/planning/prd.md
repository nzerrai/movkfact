...
Movkfact est une application web de génération de jeux de données personnalisables pour les développeurs, analystes et équipes QA. L'application propose un écran Domain Management, un wizard de création manuelle, une détection automatique de typologies via upload CSV, un éditeur de données, des APIs CRUD et un suivi d'activité complet.
...
- Fonctionnalités MVP : gestion de domaines, création de datasets (CSV ou wizard manuel), génération JSON, éditeur de données en ligne, APIs CRUD, suivi d'activité avec traçabilité.
- Hors scope MVP : IA pour matching automatique, microservices, système de plugins, modes sombre/clair.
...
- Le système réduit d'au moins 50% le temps de réponse des générations de configurations identiques successives, via mise en cache.
...
- Toutes les routes API sont protégées par authentification par token — accès refusé (HTTP 401) sans token valide.
...
## 8. Plan de livraison
- Phase 1 (Q1) : MVP — génération CSV + wizard manuel, écran Domain Management, APIs CRUD, éditeur de données, suivi d'activité.
- Phase 2 (Q2) : APIs avancées, génération par lots (jusqu'à 10 datasets parallèles), typologies additionnelles.
- Phase 3 (Q3+) : microservices, IA pour matching automatique de typologies, modes UX avancés, système de plugins.
